...
def part1(file):
    nums = [[ch for ch in line if ch.isdigit()] for line in open(file).read().split('\n') ]
    total = sum([int(num[0] + num[-1]) for num in nums]) 
    print(total)

num_dt = {"one": 1, "two": 2, "three": 3, "four": 4, "five": 5, "six": 6, "seven": 7, "eight": 8, "nine": 9}
"three eight"
"eight three"
"five eight"
""
...
def part2(file):
    ...
    searches = [re.findall(pattern, replace_line(line, overlaps)) for line in open(file).read().split('\n')]
    total = 0
    for s in searches:
        # print(s)
