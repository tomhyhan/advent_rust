...
class Grid(dict):
    ...
    def play(self, elf_death=False):
        ...
        while True:
            ...
            rounds += 1
            print(rounds)
            print(sum(unit.hp for unit in self.units if unit.team == Team.GOBLIN and unit.alive))
            print(sum(unit.hp for unit in self.units if unit.team == Team.ELF and unit.alive))
            if rounds == 77 or rounds == 78:
                print(list(unit for unit in self.units if unit.team == Team.GOBLIN))
                print(list(unit for unit in self.units if unit.team == Team.ELF))
            print()
...
lines = open('./src/input/q15.txt').read().splitlines()
